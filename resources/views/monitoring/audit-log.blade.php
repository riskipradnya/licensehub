<x-app-layout>
    <x-slot name="title">Audit Log System (CCTV)</x-slot>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 text-slate-200">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-700/50 pb-6">
            <div>
                ...
                <p class="text-sm mt-1 text-slate-400">Pantau seluruh rekam jejak aktivitas, perubahan data, dan intervensi sistem ("CCTV").</p>
            </div>
        ...
        <div class="bg-[#1E293B] border border-slate-700/50 shadow-lg rounded-xl p-6 lg:p-8">
            <div class="relative border-l border-slate-700/50 ml-4 space-y-10">
                ...
                @forelse($activities as $activity)
                    @php
                        ...
                        if ($subjectClass) {
                            ...
                            if ($subjectName) {
                                $actionText .= " : <span class='text-white font-semibold'>{$subjectName}</span>";
                            }
                        } else {
                    ...
                    <div class="relative pl-8">
                        <!-- Icon Node (Avatar) -->
                        <span class="absolute -left-4 top-1 flex h-8 w-8 items-center justify-center rounded-full border border-slate-700/50 bg-[#1E293B] shadow-sm overflow-hidden shrink-0">
                            @if($activity->causer)
                        ...
                        @if($activity->event === 'updated' && isset($activity->properties['old']) && isset($activity->properties['attributes']))
                            <div class="mt-3 bg-[#0F172A] border border-slate-700/50 rounded-lg overflow-hidden">
                                <div class="px-4 py-2 border-b border-slate-700/50 bg-slate-800/30 text-xs font-semibold text-slate-400">
                                        ...
                                        @if($oldValue !== $newValue)
                                            <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-sm">
                                                ...
                                                <div class="flex-1 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                                                    <!-- Old Value -->
                                                    <div class="flex-1 bg-red-500/10 border border-red-500/20 text-red-300 px-3 py-1.5 rounded-md break-all">
                                                        <del class="opacity-80">{{ is_array($oldValue) ? json_encode($oldValue) : ($oldValue ?: 'null') }}</del>
                                                    </div>
                                                    ...
                                                    <!-- New Value -->
                                                    <div class="flex-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 px-3 py-1.5 rounded-md break-all font-medium">
                        ...
                        @elseif(!empty($activity->properties) && count($activity->properties) > 0)
                            ...
                            <div class="mt-3 bg-[#0F172A] border border-slate-700/50 rounded-lg p-4">
                                <pre class="text-xs text-slate-400 overflow-x-auto"><code>{{ json_encode($activity->properties, JSON_PRETTY_PRINT) }}</code></pre>
                            </div>
                        @endif
                ...
                @empty
                    <div class="relative pl-8 pb-4">
                        <span class="absolute -left-4 top-1 flex h-8 w-8 items-center justify-center rounded-full border border-slate-700/50 bg-[#1E293B] text-slate-500">
