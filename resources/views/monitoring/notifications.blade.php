<x-app-layout title="Notifications & Alerts" :breadcrumbs="[['label' => 'Monitoring', 'url' => '#'], ['label' => 'Notifications']]">
    ...
    {{-- TABS --}}
    <div x-data="{ tab: new URLSearchParams(window.location.search).get('tab') || 'all' }" class="mb-6">
        ...
        <div x-show="tab === 'sent'" x-transition class="space-y-3 mt-4" style="display: none;">
            @forelse($sentNotifications as $notification)
                <div class="card flex items-start gap-4 hover:shadow-md transition-shadow bg-white border-l-4" style="border-color: var(--color-status-active);">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm mt-1" style="color: var(--color-text-secondary);">
                            {{ $notification->data['message'] ?? 'Pesan berhasil dikirim.' }}
                        </p>
                    </div>
                </div>
            @empty
                <div class="text-center py-10">
                    <svg class="w-12 h-12 mx-auto mb-3" style="color: var(--color-text-secondary); opacity: 0.5;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                    <p class="text-sm font-medium" style="color: var(--color-text-secondary);">Belum ada riwayat pesan yang terkirim.</p>
                </div>
            @endforelse
            <div class="mt-4">
                {{ $sentNotifications->appends(['tab' => 'sent'])->links() }}
            </div>
        </div>
    </div>
